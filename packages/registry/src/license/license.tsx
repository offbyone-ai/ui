...
export async function verifySession(
  config: Pick<LicenseConfig, "payUrl" | "appId">,
  sessionId: string,
): Promise<boolean> {
  const res = await fetch(
    `${config.payUrl}/api/entitlement?app=${config.appId}&session=${encodeURIComponent(sessionId)}`,
  );
...
export async function restoreByEmail(
  ...
  const res = await fetch(
    `${config.payUrl}/api/entitlement?app=${config.appId}&email=${encodeURIComponent(email)}`,
  );
  const { paid } = await res.json();
  return !!paid;
}
...
export function Paywall() {
  const { paywallOpen, paywallReason, closePaywall, restoreByEmail, startCheckout, _config } =
    useLicense();
      ...
      <DrawerContent className="max-w-lg mx-auto">
        <DrawerHeader className="flex flex-col items-center px-6 pt-6 pb-2">
          ...
          <DrawerDescription className="text-base">
            {step === "choose"
              ? (paywallReason ?? appDescription ?? `Unlock ${appName} to continue.`)
              : step === "restore"
        ...
        <div className="px-6 pb-10 pt-4">
          {step === "choose" && (
            <div className="flex flex-col gap-6">
              <Button
                className="w-full h-14 rounded-full text-base font-semibold cursor-pointer"
                disabled={submitting}
                onClick={handleBuy}
              >
                Buy —{" "}
...
export function LicenseProvider({
  ...
  const config: LicenseConfig = { appName, appId, priceId, price, payUrl, appDescription, supportUrl };
  ...
  const value: LicenseCtx = {
    ...
    requireLicense: async () => {
      ...
      const paid = identity
        ? await (identity.type === "session"
            ? verifySession(config, identity.value)
            : restoreByEmail(config, identity.value))
        : false;
      if (!paid) {
        localStorage.removeItem(key);
        setLicensed(false);
